...
The repository includes an example `docker-compose.yml`; download it and adapt values.
...
- `DELETE_ALL_ALBUMS` will delete all existing albums EVEN IF `DRY_RUN` is enabled! So by using `DELETE_ALL_ALBUMS` with `DRY_RUN`, you'll wipe all your albums and not create new ones, ending up with 0 Immich albums.
